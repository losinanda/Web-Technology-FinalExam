<x-backpage>
    <div class="w-full mt-5">
        ...
        <div class="overflow-x-auto relative sm:rounded-lg mb-4">
            <form class="form" method="get" action="{{ route('admin_searchform') }}">
                <div class="flex flex-col lg:flex-row mt-4 lg:mt-0 justify-end items-center">
                    ...
                    <div class="px-4 py-1 my-2 text-right sm:px-6">
                        ...
                        <a class="inline-flex justify-end rounded-md border border-gray-500 bg-transparent py-2 px-4 text-sm font-medium text-gray-800 shadow-sm hover:bg-gray-800 hover:text-white focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
                            href="{{ route('admin_formuser') }}">Reset</a>
                    </div>
                </div>
            </form>
        </div>
                        ...
                        @foreach ($data as $key => $d)
                            <tr class="hover:bg-grey-lighter">
                                <td class="py-4 px-6 border-b border-grey-light">
                                    {{ ($data->currentpage() - 1) * $data->perpage() + $key + 1 }}</td>
                                <td class="py-4 px-6 border-b border-grey-light">{{ $d->form_name }}</td>
                                <td class="py-4 px-6 border-b border-grey-light">
                                    {{ $d->form_phonenum }}</td>
                                </td>
                                <td class="py-4 px-6 border-b border-grey-light">
                                    ...
                                    <div class="flex">
                                        ...
                                        @elseif ($d->accept == 1)
                                            <div
                                                ...
                                                <i class="fas fa-times" style="text:white"></i>
                                            </div>
                                        @elseif ($d->accept == 2)
                                            <div
                                                class="text-white text-center bg-green-500 font-medium rounded-lg text-sm px-4 py-2.5 mr-2 mb-4">
                                                <i class="fas fa-check" style="text:white"></i>
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="py-4 px-6 border-b border-grey-light">
                                    <div class="flex">
                                        <a href="{{ route('admin_infoformuser', ['id' => $d->id_form]) }}"
                                        ...
                                        <a href="{{ route('admin_deleteformuser', ['id' => $d->id_form]) }}"
                                            onclick="return confirm('Are you sure want to delete the data?');"
                                            class="text-white text-center bg-yellow-500 hover:bg-yellow-600 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-4 py-2.5 mr-2 mb-4 dark:bg-yellow-600 dark:hover:bg-yellow-700 focus:outline-none dark:focus:ring-yellow-800"><i
